...
if (isset($_POST["createProductBtn"])) {
  ...
  if (move_uploaded_file($_FILES["imageFile"]["tmp_name"], $image)) {
    ...
    $size = $_POST["lstSize"];
    $color = $_POST["lstColor"];
    $quantity = $_POST["txtQuantity"];
    $unitPrice = $_POST["txtUnitPrice"];
    $brandId = $_POST["txtBrandId"];
    $categoryId = $_POST["txtCategoryId"];

    $sql = "INSERT INTO `product` (`name`, `description`,`image`, `size`, `color`, `quantity`, `unit_price`, `brand_id`, `category_id`) 
          VALUES ('$productName', '$description', '$dbimage', '$size', '$color', '$quantity', '$unitPrice', '$brandId', '$categoryId');";
...
if (isset($_POST["editProductBtn"])) {
  ...
  $sql = "UPDATE `product` SET `name` = '$productName', `description` = '$description', `image` = '$dbImage', `size` = '$size', `color` = '$color',
